...
**親pom の版pinを書き換えて再publishするだけで、子は pom 無変更・再ビルドのみで新しい依存を取り込む**ことを目的とする。

> 設計の背景・判断は [PLAN-cicd-trigger-children.md](../plan/PLAN-cicd-trigger-children.md) / [PLAN-emoji-plugin-sample.md](../plan/PLAN-emoji-plugin-sample.md) を参照。
> CI/CD で親→子を自動トリガする計画（ステップ5）は [PLAN-api-trigger.md](../plan/PLAN-api-trigger.md)。

## 全体構成

```
GitHub repos                         GitHub Packages (Maven Registry)
├── cap-java-emoji-plugin     ──┐    com.example.cap.plugins:emoji-plugin:1.0.0 / 1.1.0  (固定版)
├── cap-java-parent-pom-sample ─┤    com.example.central:central-parent:1.0.0-SNAPSHOT   (SNAPSHOT)
└── cap-java-child (consumer)  ──┘
        │                                   ▲
        │ <parent> 座標参照(SNAPSHOT)        │ mvn deploy
...
| 対象 | 版方式 | 役割 |
|---|---|---|
| **emoji-plugin** | **固定版**（`1.0.0` → `1.1.0`、不変） | 依存ライブラリ本体。版を上げる＝親pinを書き換える離散イベント |
| **親pom の emoji-plugin pin** | 固定版を指す | これを書き換えてpush＝「親pom更新トリガ」 |
| **親pom 自身**（子が `<parent>` 参照） | **SNAPSHOT**（`1.0.0-SNAPSHOT`、可変） | 子が `mvn -U` の単純再ビルドで新しい親を拾える |

> emoji-plugin を SNAPSHOT にすると親を触らず中身を差し替えられてしまい「親pom更新」イベントが発生しない。固定版にすることで版上げが必然的に親pom更新になる。親pom を SNAPSHOT にすることで子は pom 無変更で追従できる。
...
- 固定版運用：`pom.xml` の `<version>` を `1.0.0`（📚）/ 後で `1.1.0`（🚀）に。GitHub Packages は固定版の上書き不可（不変）なので、版差は別版 publish で表す。
...
<version>1.0.0-SNAPSHOT</version>   <!-- 親はSNAPSHOT -->
...
<properties>
  ...
  <emoji-plugin.version>1.0.0</emoji-plugin.version>      <!-- ← これを書き換える=親pom更新トリガ -->
</properties>
...
```bash
cd parent-pom
mvn clean deploy                 # central-parent:1.0.0-SNAPSHOT を publish
git commit -am "..." && git push # 親repoのソース更新（pin変更がトリガの本体）
```
...
mvn -U clean spring-boot:run  # 親更新を取り込みたいとき
```
- `clean`：target を削除しフレッシュビルド（CDS生成物も再生成）
- `-U`：SNAPSHOT 依存（＝親）をリモートに見にいって最新を取り直す
- OData の `@emoji` フィールドに 📚（emoji-plugin 1.0.0）が付けば成功
...
## ステップ5：親→子の自動トリガ（CI/CD API）

→ 別ドキュメント [PLAN-api-trigger.md](../plan/PLAN-api-trigger.md) を参照（SAP CI/CD ジョブ vs GitHub Actions の比較と設計）。
> 注：SAP CI/CD **トライアルは API 用プランが無く**、API トリガは利用不可。トライアルでは子の再ビルドを手動起動してカスケードを確認する。
...
```
① emoji-plugin の版を上げて publish    : ハンドラ変更 → pom version 1.0.0→1.1.0 → mvn clean deploy
② 親pin を上げて push & 再publish       : emoji-plugin.version 1.0.0→1.1.0 → git push → mvn clean deploy
③ 子を -U で再ビルド                    : CI/CD 再実行（or ローカル mvn -U clean ...）
   → 子pom無変更で OData が 📚→🚀
```
...
| 親更新が子に反映されない | `-U` 無しでキャッシュの古い親SNAPSHOTを使用 | `mvn -U clean ...` |
